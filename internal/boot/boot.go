...
func Run() error {
	...
	queueController, err := buildQueueController(rootCtx, startupCtx, cfg, repo)
	if err != nil {
		return err
	}
	...
	return serve(rootCtx, srv, cfg.Port)
}

func buildQueueController(rootCtx, startupCtx context.Context, cfg config, repo *db.Repository) (*controller.QueueController, error) {
	...
	return controller.NewQueueController(queueService, youtubeClient), nil
}

func serve(rootCtx context.Context, srv *http.Server, port string) error {
